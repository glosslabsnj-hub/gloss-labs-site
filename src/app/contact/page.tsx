          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
            {[
              ...
              <ScrollReveal key={card.title} delay={i * 100} direction="up">
                <a
                  href={card.href}
                  {...(card.target ? { target: card.target, rel: "noopener noreferrer" } : {})}
                  className="glass-card rounded-2xl p-8 text-center hover:border-accent/30 transition-all duration-300 cursor-pointer group block h-full"
                >
                  ...
                  <p className="text-accent font-[family-name:var(--font-body)]">{card.value}</p>
                  <p className="text-white/30 text-xs mt-2 font-[family-name:var(--font-body)]">{card.subtitle}</p>
                </a>
              </ScrollReveal>
            ))}
          </div>
                ...
                <div>
                  ...
                  <div className="space-y-4 font-[family-name:var(--font-body)]">
                    <div>
                      ...
                      <p className="text-white text-lg font-semibold">{siteInfo.address.street}</p>
                      <p className="text-white/60">{siteInfo.address.city}, {siteInfo.address.state} {siteInfo.address.zip}</p>
                    </div>
                    <div>
                      <h3 className="text-white/70 text-xs uppercase tracking-wider mb-2">Service Area</h3>
                      <p className="text-white/50 text-sm">{siteInfo.mobileService}</p>
                    </div>
                    <div>
          ...
          <ScrollReveal delay={300}>
            <div className="glass-card rounded-2xl p-8 md:p-10">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 text-sm text-white/40 font-[family-name:var(--font-body)]">
                {[
                  "Hamilton", "Trenton", "Princeton", "Lawrenceville",
                  "Ewing", "Bordentown", "Robbinsville", "East Windsor",
                  "West Windsor", "Hightstown", "Pennington", "Hopewell",
                  "Cranbury", "South Brunswick", "Plainsboro", "Monroe Township",
                ].map((area) => (
